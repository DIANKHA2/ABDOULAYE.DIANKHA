# 👋 Abdoulaye Diankha

## Software Engineer | BI Engineer | Data Engineer | Data Scientist

🎓 Master’s student in **Information Science and Technology**  
📍 **Université du Québec en Outaouais (UQO)**  

Welcome to my GitHub portfolio.  
I am a **computer engineer** with hands-on experience in designing **secure, scalable, and data-driven solutions**.  
My profile combines **software engineering, business intelligence, data engineering, and data science**, with a strong focus on **decision support, performance, and sustainable digital transformation**.

🌱 Curious, motivated, and committed to **continuous learning**, I am currently seeking a **master’s internship in Canada** where I can contribute to innovative projects, leverage data for strategic decision-making, and create **measurable business and social impact**.

---

## 🧠 Core Skills & Expertise

### 💻 Software Engineering
- Backend application development
- Microservices architecture
- Clean and maintainable code
- Web, desktop, and mobile applications

### 📊 Business Intelligence
- Decision-support systems
- Performance reporting
- BI automation
- Analytical dashboards and indicators

### 🗄️ Data Engineering
- ETL pipelines
- Data integration and transformation
- Large-scale data processing
- Data quality and consistency

### 🧠 Data Science & AI
- Text Mining & NLP
- Image processing
- Data analysis and interpretation
- Applied AI concepts

### ⚙️ DevOps & Security
- CI/CD concepts
- DevOps culture
- Secure systems
- Data access control

---

## 📂 Portfolio — Projects by Domain

All repositories are grouped by specialization to provide a **clear and recruiter-friendly overview**.

---

## 🧩 Software Engineering & Application Development

### 🔹 Projets Flutter  
📱 Cross-platform mobile development  
🔗 https://github.com/DIANKHA2/Projets-Flutter  
Mobile applications developed with Flutter, focusing on modern UI, performance, and maintainability.

---

### 🔹 Projet Django Python  
🌐 Web backend development  
🔗 https://github.com/DIANKHA2/Projet-Django-Python  
Web application developed with Django, demonstrating backend logic, routing, and MVC architecture.

---

### 🔹 Premier projet de Laravel  
🌐 PHP web application (MVC)  
🔗 https://github.com/DIANKHA2/Premier-projet-de-Laravel  
Backend web application using Laravel, illustrating routing, controllers, and structured development.

---

### 🔹 ProjetWindowsDotNet  
🪟 Desktop application  
🔗 https://github.com/DIANKHA2/ProjetWindowsDotNet  
Windows desktop application built with C# and .NET, focusing on event-driven programming.

---

### 🔹 projet-android  
📱 Native Android development  
🔗 https://github.com/DIANKHA2/projet-android  
Android application demonstrating mobile development fundamentals.

---

### 🔹 Projet d’entreprise – Application MS EMBOUTEILLAGE  
🏭 Enterprise software solution  
🔗 https://github.com/DIANKHA2/Projet-dentreprise-dapplication-MS-EMBOUTEILLAGE  
Business-oriented application designed to support industrial and organizational processes.

---

## ☁️ Backend Systems & Microservices

### 🔹 ProjetMicroServices avec Spring Boot  
☁️ Distributed systems  
🔗 https://github.com/DIANKHA2/ProjetMicroServices-avec-spring-boot  
Backend system based on microservices architecture using Spring Boot, emphasizing scalability and modularity.

---

### 🔹 backend courrier physique avec Spring Boot  
📦 Backend API  
🔗 https://github.com/DIANKHA2/backend_courrier_physique-avec-spring-boot  
Service-oriented backend for managing physical mail processes.

---

### 🔹 front-end courrier physique avec Angular  
🎨 Frontend application  
🔗 https://github.com/DIANKHA2/front-end_courrier_physique-avec-Angular  
Angular-based frontend connected to a Spring Boot backend.

---

## 📊 Business Intelligence & Decision Support

### 🔹 Mémoire Master 2 – Business Intelligence  
📘 Academic research (PDF)  
🔗 https://github.com/DIANKHA2/Mémoire-master-2-Business-intelligence  
Graduate-level thesis focused on BI systems, architectures, and decision-making processes.

---

### 🔹 MES REPORTING pour la prise de décision  
📈 Decision-support reporting  
🔗 https://github.com/DIANKHA2/MES-REPORTING-pour-la-prise-de-décision  
Reporting system designed to support performance monitoring and managerial decisions.

---

### 🔹 Projet Performance Système Décisionnel  
📊 BI performance analysis  
🔗 https://github.com/DIANKHA2/Projet_Performance_Systeme_decisionnel  
Evaluation and optimization of decision-support system performance.

---

### 🔹 Projet Business Intelligence avec automatisation personnalisée  
⚙️ Automated BI workflows  
🔗 https://github.com/DIANKHA2/projet-Bussiness-Intelligence-avec-automatisation-personnaliser  
Automation of BI processes to improve efficiency and reliability of analysis.

---

## 🗄️ Data Engineering & Integration

### 🔹 Projet d’exploitation et de valorisation des données avec Talend  
🔄 ETL & data integration  
🔗 https://github.com/DIANKHA2/Projet-dexploitation-et-de-valorisation-de-donnee-sur-Talend-Open-Studio-for-Data-Integration  
End-to-end ETL pipeline using Talend Open Studio for data integration and transformation.

---

### 🔹 projet Logiciel de mémoire licence  
📘 Academic data-focused project  
🔗 https://github.com/DIANKHA2/projet_Logiciel_de_memoire_licence  
Final-year academic project centered on data structuring and exploitation.

---

## 🧠 Data Science & Artificial Intelligence

### 🔹 Projet de Texte Mining – NLP  
🧠 Natural Language Processing  
🔗 https://github.com/DIANKHA2/Projet-de-Texte-Mining-NLP  
Text mining and NLP project focusing on preprocessing, analysis, and insight extraction from unstructured data.

---

### 🔹 Projet traitement et valorisation d’image  
🖼️ Image processing  
🔗 https://github.com/DIANKHA2/Projet-traitement-et-valorisation-dimage  
Image processing project aimed at extracting and valorizing information from visual data.

---

## 🔐 Security & Data Access Control

### 🔹 Projet de Systèmes de contrôle d’accès aux données  
🔒 Data security  
🔗 https://github.com/DIANKHA2/Projet-de-Systemes-de-controle-dacces-aux-donnees  
Implementation of access control mechanisms to protect sensitive data.

---

## ⚙️ DevOps & CI/CD

### 🔹 Mémoire DEVOPS – CI/CD (TDSI)  
🚀 DevOps practices  
🔗 https://github.com/DIANKHA2/Mémoire-DEVOPS_CICD-TDSI  
Academic work focused on CI/CD pipelines, automation, and DevOps culture.

---

### 🔹 librairy-api-flask-draft  
🧪 Python API prototype  
🔗 https://github.com/DIANKHA2/librairy-api-flask-draft  
Prototype REST API built with Flask to explore backend service development.

---

## 🎯 Career Objective

I am actively seeking a **master’s internship in Canada** that will allow me to:
- Apply my skills in **BI, data engineering, and data science**
- Contribute to **innovative and impactful digital projects**
- Support **strategic decision-making through data**
- Participate in **sustainable digital transformation initiatives**

---

## 📫 Contact

- 🌍 GitHub: https://github.com/DIANKHA2  
- 💼 LinkedIn: https://www.linkedin.com/in/elhadji-abdoulaye-diankha-053325193/ 
- 📧 Email : abdoulayediankha44@gmail.com  

---

⭐ Thank you for visiting my portfolio.  
I am open to **internships, collaborations, and innovative projects**.
